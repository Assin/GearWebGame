package kernel.display.components.tree
{
	 ...
	 *
	 */
	public class TreeCellRenderer extends Sprite implements IDispose
	{
		//鼠标作用的区域
		...
		public var nodeStatus:String = TreeView.NODE_CLOSE;
		...
		{
			this._maskX = (this._level * paddingX) * -1;
		...
		public function open():void
		{
			...
			if (this.hasChildNodes)
			{
				...
				{
					this.setOpenIcon();
					this.nodeStatus = TreeView.NODE_OPEN;
		...
		public function close():void
		{
			if (this.hasChildNodes)
			{
				if (this.isOpen() == true)
				{
					this.unSelected();
					this.setCloseIcon();
					this.nodeStatus = TreeView.NODE_CLOSE;
		...
		public function isOpen():Boolean
		{
			...
			if (this.nodeStatus == TreeView.NODE_OPEN)
			{
				return true;
			}
			
			if (this.nodeStatus == TreeView.NODE_CLOSE)
			{
				return false;
			}
			return false;
		...
		{
			if (this.hasEventListener(TreeEvent.CLICK_NODE)) 
			{
				this.removeEventListener(TreeEvent.CLICK_NODE, otherClickNode);
			}
			...
			ObjectPool.disposeObject(mouseArea);
			mouseArea = null;
			ObjectPool.disposeObject(bg);
			bg = null;
			ObjectPool.disposeObject(_data);
			_data = null;
			ObjectPool.disposeObject(_parentNode);
			_parentNode = null;
			ObjectPool.disposeObject(_childNodes);
			_childNodes = null;
			ObjectPool.disposeObject(icon);
			icon = null;
			ObjectPool.disposeObject(openIcon);
			openIcon = null;
			ObjectPool.disposeObject(closeIcon);
			closeIcon = null;
			ObjectPool.disposeObject(labelTextField);
			labelTextField = null;
			ObjectPool.disposeObject(childrenContainer);
