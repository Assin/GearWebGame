package kernel.display.components.tree
{
	 ...
	 *
	 */
	public class TreeView extends MouseEffectiveComponent
	{
		...
		/**
		 *update all cell renderer display
		 *
		 */
		...
		/**
		 *open all cellrenderer node
		 *
		 */
		public function openAllNode():void
		{
			for each (var cell:TreeCellRenderer in this.treeCellRendererList)
			{
				cell.open();
				cell.dispatchEvent(new TreeEvent(TreeEvent.CLICK_NODE, cell, true));
			}
		}
		
		/**
		 * 通过某一个属性 查找一个TreeCellRenderer
		 * @param attribute
		 * @param value
		 * @return
		 *
		 */
		public function getCellRendererByAttribute(attribute:String, value:*):TreeCellRenderer
		{
			for each (var cell:TreeCellRenderer in this.treeCellRendererList)
			{
				if (cell.data[attribute] == undefined || cell.data[attribute] == null)
				{
					continue;
				}
				if (cell.data[attribute] == String(value))
				{
					return cell;
				}
			}
		 ...
		 *
		 */
		public function selectedItem(selectCell:TreeCellRenderer):void
		{
			for each (var tcr:TreeCellRenderer in this.treeCellRendererList)
			{
				if (tcr == selectCell)
				{
					tcr.open();
					tcr.dispatchEvent(new TreeEvent(TreeEvent.CLICK_NODE, tcr, true));
				}
			}
		}
		...
		public function set dataProvider(value:*):void
		{
			...
			if (value != null)
			{
				DisplayUtil.removeAllChildren(this.treeContainer);
			}
		...
		override public function dispose():void
		{
			...
			xmlProvider = null;
			_itemRenderer = null;
			ObjectPool.disposeObject(treeCellRendererVOArrayProvider);
			treeCellRendererVOArrayProvider = null;
			ObjectPool.disposeObject(treeCellRendererArray);
			treeCellRendererArray = null;
			ObjectPool.disposeObject(treeCellRendererList);
			treeCellRendererList = null;
			ObjectPool.disposeObject(treeContainer);
			treeContainer = null;
