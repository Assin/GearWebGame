...
package kernel.runner
{
	...
	public class LogRunner
	{
		 ...
		 * default: 'CDC309AF';
		 */
		public static var password:String = 'SH';
		...
		{
			if (!ApplicationConfig.LOG_OUTPUT)
			{
				return false;
			}
			var traceContent:String = "[log]" + "[" + new Date().toLocaleString() + "]" + "[" + operation + "]" + String(value);
			trace(traceContent);
			...
			if (allowLog)
			{
				try
				{
					lc.send(TYPE + '#' + DOMAIN + CHECK + ':' + CONNECTION, operation, password, traceContent, prop);
					return true;
				} catch (e:*)
				{
					return false;
